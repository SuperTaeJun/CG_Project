...
uniform int lighting_switch;
void main()
{
	float ambientLight = 0.3;										//--- 주변 조명 계수: 0.0 ≤ ambientLight ≤ 1.
	...
	if(lighting_switch==0) FragColor = vec4 (result, 1.0);									//--- 픽셀 색을 출력
	else FragColor = vec4 (objectColor, 0.4);	
}