...
void START::animate(int y,int& _ChangeState)
{
	drop = y;
...
void GAMEOVER::init()
{
	ReadObj("asset/GAMEOVER.obj", vertexInfo);
	initBuffer(&VAO, &VBO, vertexInfo);
}

void GAMEOVER::scaleTransform(float x, float y, float z)
{
	scale = glm::mat4(1.0f);
...
void GAMEOVER::rotateTransform(float x, float y, float z, float degree)
{
	rotate = glm::mat4(1.0f);
...
void GAMEOVER::moveTranform(float x, float y, float z)
{
	move = glm::mat4(1.0f);
...
void GAMEOVER::draw(int R, int G, int B)
{
	changeColor(R, G, B);
	glBindVertexArray(VAO);
	unsigned int alpa = glGetUniformLocation(shaderID, "lighting_switch");
	glUniform1i(alpa, 0);
	scaleTransform(2.0, 5.0, 10000.0);
	//rotateTransform(1.0, 0.0, 0.0, 90);
	moveTranform(0.0, 120.0, 0.0);
	moveTranform(0.0, drop, 0.0);
	update();
	unsigned int modelTransform = glGetUniformLocation(shaderID, "modelTransform");
	glUniformMatrix4fv(modelTransform, 1, GL_FALSE, glm::value_ptr(merge));
...
void GAMEOVER::changeColor(int R, int G, int B)
{
	float r = (float)R / 255.f;
	float g = (float)G / 255.f;
	float b = (float)B / 255.f;
...
void CLEAR::init()
{
	ReadObj("asset/CLEAR.obj", vertexInfo);
	initBuffer(&VAO, &VBO, vertexInfo);
}

void CLEAR::scaleTransform(float x, float y, float z)
{
	scale = glm::mat4(1.0f);
...
void CLEAR::rotateTransform(float x, float y, float z, float degree)
{
	rotate = glm::mat4(1.0f);
...
void CLEAR::moveTranform(float x, float y, float z)
{
	move = glm::mat4(1.0f);
...
void CLEAR::draw(int R, int G, int B)
{
	changeColor(R, G, B);
	glBindVertexArray(VAO);
	unsigned int alpa = glGetUniformLocation(shaderID, "lighting_switch");
...
void CLEAR::changeColor(int R, int G, int B)
{
	float r = (float)R / 255.f;
	float g = (float)G / 255.f;
	float b = (float)B / 255.f;
